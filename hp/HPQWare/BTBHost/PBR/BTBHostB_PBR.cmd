...
SET LOGPATH=%WPDrvLetter%\system.sav\logs\CEPS\BTBHost\BTBHostB_PBR.log
...
SET FOLDER.DEST=c:\Users\Default\AppData\Local\Packages
IF EXIST "%FOLDER.SRC%" (
  MKDIR "%FOLDER.DEST%" 
...
:: Enable IE favorite bar for non-signature image
IF NOT EXIST %WPDrvLetter%\system.sav\flags\BtBR_MIC.flg (
	ECHO [%time%][%~nx0] Try to enable IE favorite bar >> %LOGPATH%
	IF EXIST %WPDrvLetter%\users\Default\ntuser.dat (
		ECHO [%time%][%~nx0] Add registry key to %WPDrvLetter%\users\Default\ntuser.dat >> %LOGPATH%
		REG LOAD HKU\DEFAULTTEMP %WPDrvLetter%\users\Default\ntuser.dat >> %LOGPATH% 2>&1
		REG ADD "HKU\DEFAULTTEMP\SOFTWARE\Microsoft\Internet Explorer\MINIE" /v LinksBandEnabled /t REG_DWORD /d 1 /f >> %LOGPATH% 2>&1
		REG UNLOAD HKU\DEFAULTTEMP >> %LOGPATH% 2>&1
	) else (
		ECHO [%time%][%~nx0] Cannot find %WPDrvLetter%\users\Default\ntuser.dat >> %LOGPATH%
	)
) else (
	ECHO [%time%][%~nx0] Signature image, no need to enable IE favorite bar >> %LOGPATH%
)
